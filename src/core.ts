...
export class RePub {
  ...
  private async checkForDRM(zip: JSZip): Promise<boolean> {
    ...
    if (encryptionFile) {
      ...
      const encryptionDoc = parser.parseFromString(
        encryptionContent,
        "text/xml"
      );
      ...
      const encryptionMethod =
        encryptionDoc.getElementsByTagName("EncryptionMethod");
        ...
        if (fullPath) {
          ...
          if (contentFile) {
            ...
            const metadataElement =
              contentDoc.getElementsByTagName("metadata")[0];
            if (metadataElement) {
              // Check for rights-related metadata
              const rights = metadataElement.getElementsByTagName("dc:rights");
              const rightsMeta = Array.from(
                metadataElement.getElementsByTagName("meta")
              ).filter((meta) => {
  ...
  async load(data: FileData): Promise<void> {
    ...
    if (await this.checkForDRM(this.zip)) {
      throw new DRMError(
        "This EPUB file is protected by DRM and cannot be modified"
      );
    }
  ...
  async open(location: string): Promise<void> {
    ...
    if (location.startsWith("http")) {
      ...
      throw new Error(
        "File path loading is not supported in browser environment"
      );
    }
  ...
  private async initialize(): Promise<void> {
    ...
    const navItem = Array.from(items).find((item) =>
    ...
    if (navItem) {
      ...
      if (href) {
        ...
        if (navFile) {
          ...
          this.navigation = parser.parseFromString(
            navContent,
            "text/xml"
          ) as unknown as XMLDocument;
        }
    ...
    if (ncxItem) {
      ...
      if (href) {
        ...
        if (ncxFile) {
          ...
          this.ncx = parser.parseFromString(
            ncxContent,
            "text/xml"
          ) as unknown as XMLDocument;
        }
  ...
  async removeContentRange(range: string): Promise<void> {
    const parseRange = (rangeStr: string): [number, number | null] => {
      ...
      if (isNaN(start) || isNaN(end)) {
        throw new Error(
          "Invalid range format. Expected numbers for start and end"
        );
      }
    ...
    const endIndex =
      end !== null ? end : Math.max(...allElements.map((el) => el.index));

    const elementsToRemove = allElements.filter(
      (el) => el.index >= start && el.index <= endIndex
    );
   ...
   */
  private async _removeContentElement(
    ...
    const manifestItems = Array.from(
      this.manifest.getElementsByTagName("item")
    );
  ...
  private getChapterBoundaries(): Map<string, string[]> {
    ...
    for (let i = 0; i < navPoints.length; i++) {
      ...
      boundaries.set(chapterId, spineItems.slice(spineIndex, endIndex));
    }
   ...
   * @param identifier String ID or numeric index of the element to remove
   */
  private async _removeContentElementWithSplits(
    identifier: string | number
  ): Promise<void> {
    const content =
      typeof identifier === "number"
    ...
    for (const id of relatedIds) {
      const item = Array.from(this.manifest!.getElementsByTagName("item")).find(
      ...
      if (item) {
        ...
        // Remove from spine
        const spineItem = Array.from(
          this.spine!.getElementsByTagName("itemref")
   ...
   */
  private findContentElementByIndex(
    index: number,
  ...
  async removeExcept(keep: (string | number)[]): Promise<void> {
    ...
    // Get all content IDs
    const allIds = new Set(this.contents.map((element) => element.id));
   ...
   */
  async removeExceptWhere(
    filterFn: (element: ContentElement) => boolean
  ): Promise<void> {
    // Find all elements that don't match the filter
    const elementsToRemove = this.contents.filter(
      (element) => !filterFn(element)
    );
   ...
   * @returns String with entities decoded to Unicode characters
   */
  ...
  private updateMetadata(): void {
    ...
    let modifiedElement = Array.from(
  ...
  protected async prepareOutput(): Promise<void> {
    ...
    // Modified saveXmlContent function to use he for entity decoding
    const saveXmlContent = (
      ...
      let content = serializer.serializeToString(
    ...
    // Process navigation document if it exists
    if (this.navigation) {
    ...
    // Process NCX if it exists
    if (this.ncx) {
      const ncxItem = Array.from(
        this.manifest.getElementsByTagName("item")
      ).find(
        (item) => item.getAttribute("media-type") === "application/x-dtbncx+xml"
      );
   ...
   */
  async getOutput(
    type: "blob" | "arraybuffer" | "uint8array" | "base64" = "blob"
  ): Promise<any> {
   ...
   */
  // Modify content insertion methods to use he for entity decoding
  private createXhtmlWrapper(
    content: string,
    title?: string,
    css?: string
  ): string {
    const safeContent = this.decodeHtmlEntities(content);
    const safeTitle = title ? this.decodeHtmlEntities(title) : "New Content";
  ...
  // Add entity handling to content processing methods
  private async processContentFile(content: string): Promise<string> {
   ...
   */
  private async insertContentAtIndex(
    content: string,
    index: number,
    ...
    const htmlContent =
      options.type === "md" ? this.md.render(content) : content;
    ...
    const xhtmlContent = this.createXhtmlWrapper(
      htmlContent,
      options.title,
      options.css
    );
    ...
    if (this.contents && index < this.contents.length) {
      ...
      const referenceManifestItem = Array.from(
        this.manifest.getElementsByTagName("item")
      ).find(
  ...
  getMetadata(): FullMetadata {
    ...
    const metadataElement =
      this.manifest.ownerDocument.getElementsByTagName("metadata")[0];
  ...
  getCoreMetadata(): CoreMetadata {
     ...
     */
    const getFirstValue = (
      prop: MetadataProperty | MetadataProperty[] | undefined
    ): string => {
      if (!prop) return "";
    ...
    if (Array.isArray(metadata["title"])) {
      const subtitleEntry = (metadata["title"] as MetadataProperty[]).find(
        (t) => t.refinements?.["title-type"] === "subtitle"
      );
      if (subtitleEntry) {
    ...
    return {
      ...
      date:
        getFirstValue(metadata["date"]) ||
        getFirstValue(metadata["dcterms:modified"]) ||
        null,
    };
  }
  ...
  private getCoverItem(): XMLElement | null {
    ...
    if (!coverItem) {
      const guide =
        this.manifest.ownerDocument.getElementsByTagName("guide")[0];
      if (guide) {
  ...
  async getCover(): Promise<CoverImage | null> {
    ...
    if (coverItem) {
      ...
      if (href && mediaType) {
        const coverPath = this.path.join(
          this.path.dirname(this.contentPath),
          href
        );
  ...
  async findOrphanedMedia(remove: boolean = false): Promise<string[]> {
    ...
    for (const item of contentItems) {
      ...
      mediaMap.forEach((media, id) => {
        ...
        const relativePath = normalizePath(
          this.path.join(this.path.dirname(href), mediaPath)
        );
    ...
    const orphanedItems = Array.from(mediaMap.values()).filter(
  ...
  async getAsset(href: string): Promise<{
    ...
    const assetPath = this.path.join(
      this.path.dirname(this.contentPath),
      assetHref
    );
